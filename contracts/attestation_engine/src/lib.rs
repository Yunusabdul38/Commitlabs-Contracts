...
impl AttestationEngineContract {
    ...
    pub fn record_fees(e: Env, commitment_id: u32, fee_amount: i128) {
        if fee_amount <= 0 {
        ...
        Self::attest(e.clone(), commitment_id, String::from_str(&e, "fee_generation"), data, admin.clone());

        e.events().publish((symbol_short!("fees"), commitment_id), (fee_amount, new_total));
    }

    /// Record drawdown event
    pub fn record_drawdown(e: Env, commitment_id: u32, drawdown_percent: i128) {
        let mut data = Map::new(&e);
        data.set(String::from_str(&e, "drawdown"), String::from_str(&e, "drawdown"));
        let is_compliant = drawdown_percent <= 100;
        let admin: Address = e.storage().instance().get(&symbol_short!("ADMIN")).unwrap();
        Self::attest(e.clone(), commitment_id, String::from_str(&e, "drawdown"), data, admin);

        let _metrics = Self::get_health_metrics(e.clone(), commitment_id);
